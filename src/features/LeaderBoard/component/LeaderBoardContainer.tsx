    ...
    <div className="flex flex-col items-center">
      <div className="relative w-fit h-fit">
        ...
        <div className="absolute inset-0 flex justify-center items-center m-10 flex-col pb-7">
          <div className="bg-white h-64 w-full overflow-y-auto p-4 rounded-lg">
            <ol className="text-[#7920FF] list-decimal font-sourceCodeFont text-xl">
              {players.map((player: any, index: number) => {
                ...
                return (
                  <li
                    key={player.id}
                    className={`mb-2 ${isCurrentUser ? "font-bold text-[#C88D00]" : ""} list-none`}
                  >
                    {index + 1}. {player.name || "Player"} — <span className="font-bold">{player.score || 0}</span> pts
                    {index === 0 && " 👑"}
                  </li>
                );
              })}
          ...
          <p className="text-[#7920FF] font-game text-xl pt-3 text-center">
            Top 5 Players
          </p>
        </div>
