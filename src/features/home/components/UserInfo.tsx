...
export default function UserInfo() {
  const { data, isLoading, isError } = useUserData() as { 
    data: UserData; 
    isLoading: boolean; 
    isError: boolean;
  };
  ...
  if (isError || !data) return <p className="text-red-500 font-game">Error loading user data</p>;

  const { name, score, visited_booths = [] } = data;
